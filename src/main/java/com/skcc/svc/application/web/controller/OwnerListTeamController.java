...
public class OwnerListTeamController {
    ...
    @GetMapping("/owner/list-team/{cid}")
    public ModelAndView TeamListStorePage(Model model, @PathVariable String cid){
    	...
    	for(int i=0; i<balanceInfoList.size(); i++)
    	{
    		...
    		CustomerResponseDTO c = customerService.getOneCustomer(balanceInfo.getCid());
    		customerNameList.add(c.getCname());
    	}
    	
    	ModelAndView modelAndView = new ModelAndView("owner/owner_list_team");
